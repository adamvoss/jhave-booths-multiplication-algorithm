...
\section{Multiplication by Repeated Additions} %If this is too informal feel free to change it
...
%55: Additionally, because of the repeated use of addition, this multiplication is guaranteed to be significantly more computationally complex than that of binary addition.
...
%I want to leave this out since it might imply that we will develop something that isn't significantly more complex than addition.
...
It involves multiplying two numbers: the multiplicand, $M$, by successively more significant digits of the multiplier, $Q$.%Consistancy!
...
We will see how these additional multiplications are not a problem for a computer in the next section.%I liked the discussion of M * 0 => 0, M * 1 => M here. I don't think the student will be too confused here, so there's no reason to put the discussion off for later. It's not technically a computer issue
...
We could instead choose to always examine the \emph{lsb} of register Q, and for every right shift operation on register A, we would also do a right shift on register Q. %Thus, when the algorithm finishes, all of the previous values in Q will have been discarded.
