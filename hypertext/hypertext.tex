...
Despite working for multiplying signed numbers, this method has the undesired characteristic of having wildly varying execution time tied to the magnitude of the numbers being multiplied.
Additionally, because of the repeated use of addition, this multiplication is guaranteed to be significantly more computationally complex than that of binary addition.
...
We do not need to do repeated additions when we do decimal multiplication by hand.
Hopefully we can use insight from our experience doing decimal multiplication to find a more efficient way do to do binary multiplication with hardware.

%This section seems math-y, maybe should be re-arranged in the article
\section{Multiplication by Partial Products}
Multiplication by partial products is what most people use to do long multiplication.
...
%remove the leading zero from this diagram % add the final times 0 multiplication?
...
As most ALUs are only capable of adding two numbers together at a time, a \emph{running product} must be use to store the partial products instead of saving the summation for the end of the process. %Valid question, What's an ALU?
...
\begin{comment} %already addressed
