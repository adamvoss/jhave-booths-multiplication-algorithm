...
\title{Multiplication of Signed Binary Numbers}
\date{\today}
...
To follow this section you will need a familiarity the two's complement representation of signed numbers.
...
Some background on machine architecture, such as registers and primitive operations, will be helpful.
%While some references are made to decimal multiplication methods, these are explained in this text and the reader need no prior understanding. :P
...
%wordy, consider revision
For information regarding the use of this tool, consult ``A Guide for Visualizing Booth's Multiplication Algorithm''.
%add URL

\section{Lot's of Additions} %If this is too informal feel free to change it
Multiplication is often defined recursively as repeated additions.
%you may add a formal definition here if you like, it would be very appropriate
To calculate $11 \cdot 23$, you start with 0 and add 11 to it 23 times.
However, the numerically equivalent problem of calculating $23 \cdot 11$ requires and additional twelve fewer.
You could do a comparison to ensure the multiplier is always the smaller number, but on average there will still be a lot of addition operations.
%Could be omitted I suppose.

This works equally well for binary numbers: %This might flow better if I didn't go back and forth between Binary, Decimal
\begin{equation}
0 + 1111 + 1111 + 1111 = 1101 = 1111 \cdot 0011
\end{equation}

Despite working for multiplying signed numbers, this method has the undesired characteristic of has wildly varying execution times tied to the magnitude of the numbers being multiplied.
Additionally because of the repeated use of addition, this multiplication is grunted to be significantly more computationally complex than that of binary addition.
%We say computationally complex so I am very tempted to state the complexity classification Big-O of n where n is the multiplier (or the lesser of the multiplicand, multiplier)

Computer architects generally prefer predictability in execution times.
We don't need to do repeated additions when ever we do multiplication, so hopefully we can use that insight to find a more efficient way do to do binary multiplication.

%This section seems math-y, maybe should be re-arranged in the article
\section{Multiplication by Partial Products}
Multiplication by partial products is what most people use when they do long multiplication.
It involves multiplying two numbers: the multiplicand,$M$, by successively more significant digits of the multiplier, or $Q$.
Each of these individual multiplications is referred to as a ``partial product''. The sum of these partial products is the final product.

%Picture or equation here

Since that works works well for us with decimal numbers, lets try it with binary.
Consider the following calculation of the multiplication of 01011 and 010111, which are the numbers 11 and 23 respectively,  in two's complement notation.

%diagram goes here
\begin{figure}[h]
\centering
\includegraphics[scale=0.7]{saam3.pdf} %remove the leading zero from this diagram % add the final times 0 multiplication?
\caption{Multiplication by Partial Products}
\end{figure}

While this looks simpler, because there are now only five addition operations compared to the 23 we saw using repeated additions.
However, we have now have six multiplications which may seem like we are making our problem worse.
We will see how these multiplications are not a problem a computer in the next section.


An astute reader may have realized that this does not work directly for negative numbers when they are in two's complement notation.
In that case we can simply change the numbers to their positive representation and do the multiplication.
...
\begin{comment} %moved because this was binary information mixed in with decimal talk
...
We can then express the problem $M \cdot Q$ as $M \cdot (q_{n-1} \cdot 2^{n-1} + q_{n-2} \cdot 2^{n-2} + \ldots + q_{0} \cdot 2^{0})$.
Each one of the products $(q_i \cdot 2^{i}) \cdot M$ is a partial product.
For binary values, there are only two values a partial product may be: if $q_i = 0$, then the result is simply $0$; otherwise the result is $M \cdot 2^i$.
\end{comment}



\section{Shift-and-Add: The Hardware-friendly Implementation} %be sure to state how XOR of the MSB provides whether the result is positive or negative
