...
Another way to say this is that the final product of $M \cdot Q$ is the sum of all $M \cdot q_i$, where $M$ is the multiplicand, $Q$ is the multiplier, $q_i$ is bit $i$ from Q and $i$ ranges from 0 to one less than the number of digits in Q.
...
This means a running product must be calculated for every partial product, instead of at the very end of the process.
...
This is also known as an \emph{arithmetic shift right}.
...
Here is an improved version of the shift-and-add multiplication algorithm, with the proposed changes: %diagram goes here
...
For use in binary multiplication, this equality translates into the following: for every string of 1s in the multiplier (Q), replace the rightmost (least significant) 1 with $\bar{1}$, replace the first 0 on the left of the string (if it exists) with 1, and set all the 1s between the positions of the bits you just modified to 0.
...
In the second case, the most significant bit is converted to $\bar{1}$, representing a subtraction of the value in multiplicand (M).
...
In the third case, consider the largest leftmost ``string'' of 1s in Q (this is, in fact, the string which indicates the binary value is negative).
By Booth's method, the rightmost bit in this string will be converted into $\bar{1}$, since it is either followed by a 0 or else not followed by anything (if it were followed by a 1, then the string we selected would not be the \emph{largest} leftmost string of 1s).
...
where $n_i$ is value of the $i$th bit of $N$, and $k$ is the \emph{position} (not value) of the rightmost bit in the largest leftmost string of 1s.
Similarly, the encoded value of $N$, which we will call $N^{\prime}$, can be expressed as:
\[-2^{k^{\prime}} + \sum_{i=0}^{k-1} n_i^{\prime} \cdot 2^i\]
where $n_i^{\prime}$ is the value of the $i$th bit of $N^{\prime}$.
From our discussion of case three, we know the position of the rightmost bit in the largest leftmost string of 1s in $N$ is the same as the position of the leftmost bit with value $\bar{1}$ in $N^{\prime}$, so we know that $k$ and $k^{\prime}$ are equal.
...
Scanning a binary number from right to left, the pair `10' signifies the beginning of a string of 1s and thus corresponds to a subtraction (or $\bar{1}$).
The pair `01' signifies the end of a string of 1s and corresponds to an addition.
